...
mod tests {
  ...
  use crate::{
    background::ThreadPool, metrics::MetricsRegistry, table::TableName, DefaultIOBackend,
  };
  ...
  fn test_save_and_load() {
    ...
    let io_pool =
      IOPool::with_backend(DefaultIOBackend, dir.path(), &thread_pool, metrics).unwrap();
