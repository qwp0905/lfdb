...
impl Operation {
  ...
  fn byte_len(&self) -> u16 {
    1 + match self {
      ...
      Self::Checkpoint(_, path) => LOG_ID_BYTES as u16 + path.as_os_str().len() as u16,
      _ => 0,
    }
  }
}
...
impl LogRecord {
  ...
  pub const fn new_checkpoint(
    ...
    Self::new(log_id, 0, Operation::Checkpoint(last_log_id, snapshot_path))
  }
  ...
  unsafe fn write_at(&self, ptr: *mut u8) {
    ...
    *ptr.add(offset) = self.operation.type_byte();
    ...
    match &self.operation {
      ...
      Operation::Checkpoint(log_id, path) => {
  ...
  fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
    ...
    let operation = match unsafe { *ptr.add(offset - 1) } {
      ...
      5 => unsafe {
        ...
        Operation::Checkpoint(log_id, path.into())
      },
      6 => unsafe {
        let table_id =
          TableId::from_le_bytes((ptr.add(offset) as *const [u8; TABLE_ID_BYTES]).read());
